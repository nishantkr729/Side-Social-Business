...
function BusinessLayout({ children }) {
  ...
  return <div className="min-h-[100dvh] flex flex-col bg-[#f8fafb] text-gray-900 font-sans">
    ...
    key={item.href}
    href={item.href}
    className={`text-sm font-medium transition-colors ${location === item.href ? "text-teal-600" : "text-gray-500 hover:text-teal-600"}`}
  >
                  {item.label}
                </Link>)}
            </nav>
      ...
      <footer className="bg-teal-950 text-teal-100 py-12 mt-auto">
        <div className="container mx-auto px-4 md:px-8 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="text-sm text-teal-300/60">
            &copy; {(/* @__PURE__ */ new Date()).getFullYear()} Omnis. Strategy with purpose.
          </div>
        </div>
      </footer>
    </div>;
