...
function HomeFooter() {
  return <footer className="border-t border-white/10 mt-auto">
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 mb-16">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {socialLinks.map((item) => <li key={item.label}>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {businessLinks.map((item) => <li key={item.label}>
                  <Link href={item.href} className="text-sm text-gray-400 hover:text-white transition-colors">
                    {item.label}
                  </Link>
                </li>)}
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-gray-600">&copy; {(/* @__PURE__ */ new Date()).getFullYear()} Omnis. All rights reserved.</p>
